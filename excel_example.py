...
Excel
    * Excel is the best application to verify and analyze results, spreadsheets beat any text editor
...
def instantiate_lev(ws_name):
    ...
    Instantiating a new lev_cls will ALWAYS clear the Autofilter
    ...
    # lev_cls range reference syntax
...
def write_values():
    ...
    # the lev_cls.__setitem__ protects against this issue
...
def allow_worksheet_focus():
    """
    if lev_cls.allow_focus = False, lev.activate() will have no effect
    ...
    lev_cls.allow_focus = False


def activate_all_worksheets(allow_focus):
    lev_cls.allow_focus = allow_focus
    print('lev_cls.allow_focus = {}'.format(allow_focus))
